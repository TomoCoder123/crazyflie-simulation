                                             ...
                                             //(float)pos.x, (float)pos.y, rssi_beacon_filtered, rssi_inter_ext, rssi_angle_inter_ext, priority, outbound
{
  ...
  if (state == 1){
    if (front_range < ref_distance_from_wall + 0.2f) {
      ...
      wallFollowerInit(0.4, 0.5, 3); //make wall follower function
  ...
  if (state == 1){
   ...
   if (direction == -1) {
      state_wf = wallFollower(&temp_vel_x, &temp_vel_y, &temp_vel_w, front_range, left_range, current_heading, direction); //make wallfollow function
    } else {
